...
class MultiShapeDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(masks.shape[0]):
            current_mask = masks[i].astype(bool) & (~visible)
            if not np.any(current_mask):
                continue

            masks[i] = current_mask.astype(masks.dtype)
            visible |= current_mask

            pos = np.where(current_mask)
            kept_masks.append(masks[i])
            kept_boxes.append(np.asarray([np.min(pos[1]), np.min(pos[0]), np.max(pos[1]), np.max(pos[0])]))
            kept_ids.append(obj_ids[i])
            kept_params.append(obj_param[i])

        masks = np.asarray(kept_masks)
        boxes = np.asarray(kept_boxes)
        obj_ids = np.asarray(kept_ids)
        obj_param = np.asarray(kept_params, dtype=int)
