...
def bn_backward(dout, cache):
    ...
    dsample_mean = np.sum(dx_hat * -1.0 / np.sqrt(sample_var + eps), axis=0) + dsample_var * np.sum(-2.0 * (x - sample_mean), axis=0) / N
    dx = dx_hat / np.sqrt(sample_var + eps) + dsample_var * 2.0 * (x - sample_mean) / N + dsample_mean / N
...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        mean = momentum * mean + (1 - momentum) * cache[2]
        var = momentum * var + (1 - momentum) * cache[3]
