...
class CIFAR10(torch.utils.data.Dataset):
    ...
    def __init__(self, train=True):
        ...
        self.pos_augment = tfs.Compose([
            tfs.ToPILImage(),
            tfs.RandomHorizontalFlip(p=0.5),
        ])

        self.color_augment = tfs.Compose([
            tfs.ColorJitter(brightness=0.1, contrast=0.5, saturation=0.5, hue=0.2),
            tfs.ToTensor(),
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            pil_img = self.pos_augment(img)
            out_tensor = self.color_augment(pil_img)
            ...
            t = test_transform(pil_img).numpy().astype(np.float32)
            return t, target
...
if __name__ == '__main__':
    ...
    # position augmentation (symmetrical / horizontal flip)
    aug1 = img.copy()
    
    aug1_transform = tfs.Compose([
        tfs.RandomHorizontalFlip(p=1.0)
    ])
    aug1 = aug1_transform(aug1)
    aug1.save(f'../results/Lenna_aug1.png')

    aug2 = img.copy()
    
    ca = tfs.ColorJitter(brightness=0.3, contrast=0.5, saturation=0.5, hue=0.2)
    aug2 = ca(aug2)
