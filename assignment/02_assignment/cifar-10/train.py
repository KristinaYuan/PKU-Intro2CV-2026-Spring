...
def MyCELoss(pred, gt):
    pred_shifted = pred - torch.max(pred, dim=1, keepdim=True).values
    log_probs = pred_shifted - torch.log(torch.sum(torch.exp(pred_shifted), dim=1, keepdim=True))
    
    batch_size = pred.shape[0]
    loss = -log_probs[torch.arange(batch_size), gt].mean()
    return loss 
